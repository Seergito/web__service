...
elseif ($_SERVER['REQUEST_METHOD'] === 'GET'){ // SELECT
	...
	elseif (count($_GET)==1 && isset($num_empleado) && $num_empleado!=null) {
	 $where= " WHERE NUM_EMPLEADO ='$num_empleado' ";
	}
			// Búsqueda por nombre y localidad. Dato enviados en el URL.
	elseif (count($_GET)==2 && isset($nombre_empleado) &&  $nombre_emplado!=null && isset($puesto) && $puesto!=null) {
	 $where = " WHERE NOMBRE_EMPLEADO='$nombre_empleado' and PUESTO='$puesto'";
	}
	else {
		die_por_fallo_en_sintaxis_peticion();
	}

	$consulta = "SELECT * FROM `empleados` $where order by NOMBRE_EMPLEADO"; 
